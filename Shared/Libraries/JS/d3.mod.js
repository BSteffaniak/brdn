d3 = function() {
  ...
  d3.behavior.drag = function() {
    var event = d3_eventDispatch(drag, "drag", "dragstart", "mouseup", "dragend"), origin = null, inertia = false, damping = -.005, newDrag = false, samples = 8, xs = [], ys = [], ts = [], x, y, vx, vy, t;
    function drag() {
    ...
    function mousedown() {
      ...
      var w = d3.select(d3_window).on(touchId != null ? "touchmove.drag-" + touchId : "mousemove.drag", dragmove).on(touchId != null ? "touchend.drag-" + touchId : "mouseup.drag", mouseup, true);
      ...
      function dragmove() {
        if (!target.parentNode) return mouseup();
      ...
      function mouseup() {
        ...
        if (inertia) {
          ...
          var last = xs.length - 1, dt = ts[last] - ts[0];
